...
/* Версия 1.6
  - добавлено: 5 нажатий на сенсор + удержание - включение калибровки сенсора CO2
*/
...
bool CALIBRATION = false;                   // включение/выключение калибровки датчика CO2

// управление яркостью
byte LED_BRIGHT = 1;         // при отсутствии сохранения в EEPROM: яркость светодиода СО2 (0 - 10) (коэффициент настраиваемой яркости индикатора по умолчанию, если нет сохранения и не автоматическая регулировка (с)НР)
byte LCD_BRIGHT = 1;         // при отсутствии сохранения в EEPROM: яркость экрана (0 - 10) (коэффициент настраиваемой яркости экрана по умолчанию, если нет сохранения и не автоматическая регулировка (с)НР)
...
#define BRIGHT_THRESHOLD 500  // величина сигнала, ниже которой яркость переключится на минимум (0-1023)
#define LED_BRIGHT_MAX 30    // макс яркость светодиода СО2 (0 - 255)
#define LED_BRIGHT_MIN 0     // мин яркость светодиода СО2 (0 - 255)
#define LCD_BRIGHT_MAX 100    // макс яркость подсветки дисплея (0 - 255)
...
#define maxHum 18       // и выше - синий
#define normHum 8      // и ниже - розовый
#define minHum 6       // и ниже - красный
#define blinkLEDHum 4  // Значение влажности, при показаниях ниже порога которого будет мигать индикатор

#define normPress 733  // и ниже - розовый
#define minPress 728   // и ниже - красный

#define minRain -50   // и ниже - красный
#define normRain -20  // и ниже - розовый
...
byte podMode = 0;  // подрежим меню(с)НР
byte mode0scr = 0;
/* (с)НР
  0 - Крупно время
...
// график
//смотри createchar
uint8_t rowS[8] = { 0b00000, 0b00000, 0b00000, 0b00000, 0b10001, 0b01010, 0b00100, 0b00000 };  // loadPlot стрелка вниз (с)НР
uint8_t row7[8] = { 0b00000,  0b11111,  0b11111,  0b11111,  0b11111,  0b11111,  0b11111,  0b11111 }; // loadPlot может столбцы?
uint8_t row6[8] = { 0b00000,  0b00000,  0b11111,  0b11111,  0b11111,  0b11111,  0b11111,  0b11111 }; // 6loadPlot может столбцы?
uint8_t row5[8] = { 0b00000, 0b00000, 0b00000, 0b11111, 0b11111, 0b11111, 0b11111, 0b11111 };  // в т.ч. для четырехстрочных цифр 2, 3, 4, 5, 6, 8, 9, 0 (с)НР нижние палки 5х5
uint8_t row4[8] = { 0b00000, 0b00000, 0b00000, 0b00000, 0b11111, 0b11111, 0b11111, 0b11111 };  // loadPlot правая вертикальная полоса на графике?
uint8_t row3[8] = { 0b00000, 0b00000, 0b00000, 0b00000, 0b00000, 0b11111, 0b11111, 0b11111 };  // в т.ч. для двустрочной цифры 0, для четырехстрочных цифр 2, 3, 4, 5, 6, 8, 9 (с)НР нижние палки ?x5
uint8_t row2[8] = { 0b00000, 0b00000, 0b00000, 0b00000, 0b00000, 0b00000, 0b11111, 0b11111 };  // 0 2loadPlot в т.ч. для двустрочной цифры 4 (с)НР нижняя палка 2х5?
uint8_t row1[8] = { 0b00000, 0b00000, 0b00000, 0b00000, 0b00000, 0b00000, 0b00000, 0b11111 };  // нижняя полоса на графике

// цифры //  (с)НР
uint8_t UB[8] = { 0b11111, 0b11111, 0b11111, 0b00000, 0b00000, 0b00000, 0b00000, 0b00000 };   // 2 для двустрочных 7, 0   // для четырехстрочных 2, 3, 4, 5, 6, 8, 9 верхняя палка 3х5
uint8_t UMB[8] = { 0b11111, 0b11111, 0b11111, 0b00000, 0b00000, 0b00000, 0b11111, 0b11111 };  // 3 для двустрочных 2, 3, 5, 6, 8, 9 верхняя палка 3х5
uint8_t LMB[8] = { 0b11111, 0b00000, 0b00000, 0b00000, 0b00000, 0b11111, 0b11111, 0b11111 };  // 4 для двустрочных 2, 3, 5, 6, 8, 9 // нижняя 3х5
uint8_t LM2[8] = { 0b11111, 0b00000, 0b00000, 0b00000, 0b00000, 0b00000, 0b00000, 0b00000 };  // 5 для двустрочной 4 верхняя палка 1х5
uint8_t UT[8] = { 0b11111, 0b11111, 0b11111, 0b11111, 0b11111, 0b00000, 0b00000, 0b00000 };   // 0 для четырехстрочных 2, 3, 4, 5, 6, 7, 8, 9, 0 верхняя палка 5х5
uint8_t full[8] = { 0b11111, 0b11111, 0b11111, 0b11111, 0b11111, 0b11111, 0b11111, 0b11111 }; // 6 все закрашиваем

uint8_t KU[8] = { 0b00000, 0b00000, 0b00000, 0b00001, 0b00010, 0b00100, 0b01000, 0b10000 };  // 4 для четырехстрочных /
uint8_t KD[8] = { 0b00001, 0b00010, 0b00100, 0b01000, 0b10000, 0b00000, 0b00000, 0b00000 };  // 5 для четырехстрочных / в другом месте
...
void drawDig(byte dig, byte x, byte y) {  // рисуем цифры (с)НР ---------------------------------------
  if (bigDig && DISPLAY_TYPE == 1) {
    switch (dig) {  // четырехстрочные цифры (с)НР
      case 0:
        digSeg(x, y, 6, 0, 6, 6, 32, 6);
        digSeg(x, y + 2, 6, 32, 6, 6, 3, 6);
        break;
      case 1:
        digSeg(x, y, 32, 6, 32, 32, 6, 32);
        digSeg(x, y + 2, 32, 6, 32, 32, 6, 32);
        break;
      case 2:
        digSeg(x, y, 0, 0, 6, 1, 1, 6);
        digSeg(x, y + 2, 6, 2, 2, 6, 3, 3);
        break;
      case 3:
        digSeg(x, y, 0, 0, 6, 1, 1, 6);
        digSeg(x, y + 2, 2, 2, 6, 3, 3, 6);
        break;
      case 4:
        digSeg(x, y, 6, 32, 6, 6, 1, 6);
        digSeg(x, y + 2, 2, 2, 6, 32, 32, 6);
        break;
      case 5:
        digSeg(x, y, 6, 0, 0, 6, 1, 1);
        digSeg(x, y + 2, 2, 2, 6, 3, 3, 6);
        break;
      case 6:
        digSeg(x, y, 6, 0, 0, 6, 1, 1);
        digSeg(x, y + 2, 6, 2, 6, 6, 3, 6);
        break;
      case 7:
        digSeg(x, y, 0, 0, 6, 32, 32, 6);
        digSeg(x, y + 2, 32, 6, 32, 32, 6, 32);
        break;
      case 8:
        digSeg(x, y, 6, 0, 6, 6, 1, 6);
        digSeg(x, y + 2, 6, 2, 6, 6, 3, 6);
        break;
      case 9:
        digSeg(x, y, 6, 0, 6, 6, 1, 6);
    ...
    switch (dig) {  // двухстрочные цифры
      case 0:
        digSeg(x, y, 6, 1, 6, 6, 2, 6);
        break;
      case 1:
        digSeg(x, y, 32, 6, 32, 32, 6, 32);
        break;
      case 2:
        digSeg(x, y, 3, 3, 6, 6, 4, 4);
        break;
      case 3:
      ...
      case 5:
        ...
        break;
      case 6:
        digSeg(x, y, 6, 3, 3, 6, 4, 6);
        break;
      case 7:
        digSeg(x, y, 1, 1, 6, 32, 6, 32);
        break;
      case 8:
        digSeg(x, y, 6, 3, 6, 6, 4, 6);
        break;
      case 9:
        digSeg(x, y, 6, 3, 6, 4, 4, 6);
        break;
      case 10:
        digSeg(x, y, 32, 32, 32, 32, 32, 32);
        break;
...
void drawTemp(float dispTemp, byte x, byte y) {  // Температура крупно на главном экране (с)НР ----------------------------
  ...
  if (bigDig && DISPLAY_TYPE == 1) {
    ...
    lcd.write(1);  // десятичная точка на большом
  } else {
    lcd.setCursor(x + 7, y + 1);
    lcd.write(46);  // десятичная точка на малом экране
  }
  lcd.setCursor(x + 13, y);
  lcd.write(79);  // градусы
}

void drawHum(int dispHum, byte x, byte y) {  // Влажность крупно на главном экране (с)НР ----------------------------
  //if (dispHum / 100 == 0) drawDig(10, x, y); //для 100% влажности
  //else drawDig(dispHum / 100, x, y); // первая цифра
  //if ((dispHum % 100) / 10 == 0) drawDig(0, x + 4, y); //для 100% влажности
  //else drawDig(dispHum / 10, x + 4, y); // вторая
  //drawDig(int(dispHum) % 10, x + 8, y); // последняя

  if(dispHum < 10) {
    drawDig(dispHum, x + 8, y);
  }
  else if (dispHum >= 10) {
    drawDig((int) (dispHum / 10), x + 4, y);
    drawDig(dispHum % 10, x + 8, y);
  }
  if (bigDig && DISPLAY_TYPE == 1) {
    //процент после влажности
    lcd.setCursor(x + 12, y + 2);
    lcd.print("\6\2");
    lcd.setCursor(x + 12, y + 3);
    lcd.print("\6");
  } else {
  ...
  for (byte i = 0; i < width; i++) {  // каждый столбец параметров
    ...
    for (byte n = 0; n < height; n++) {  // для всех строк графика
      ...
      if (n >= infill) {  // если достигли уровня
        ...
        else lcd.write(32);                      // если дробные == 0, заливаем пустой
...
void setLED() {
  ...
  // ниже задается цвет индикатора в зависимости от назначенного сенсора: красный, розовый, зеленый, синий (с)НР
  if (CALIBRATION == true) setLEDcolor(48); // синий если калибруем датчик CO2
  else if ((dispCO2 >= maxCO2) && LEDType == 0 || (dispHum <= minHum) && LEDType == 1 || (dispTemp >= maxTemp) && LEDType == 2 || (dispRain <= minRain) && LEDType == 3 || (dispPres <= minPress) && LEDType == 4) setLEDcolor(4);                // красный
  else if ((dispCO2 >= normCO2) && LEDType == 0 || (dispHum <= normHum) && LEDType == 1 || (dispTemp >= normTemp) && LEDType == 2 || (dispRain <= normRain) && LEDType == 3 || (dispPres <= normPress) && LEDType == 4) setLEDcolor(19);  // розовый
  else if (LEDType == 0 || (dispHum <= maxHum) && LEDType == 1 || (dispTemp >= minTemp) && LEDType == 2 || (dispRain <= maxRain) && LEDType == 3 || LEDType == 4) setLEDcolor(13);                                                           // зеленый
  else setLEDcolor(48);        
}
...
void setup() {
  ...
  if (CALIBRATION == false) mhz19.setAutoCalibration(false);
